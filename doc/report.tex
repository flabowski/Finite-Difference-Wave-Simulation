...
u[i, j, n+1] = 1/(b*dt + 2)*(t1 + 2*t2 + 2*t3)
        ...
        This means:\\ \\
        \begin{multiline}
        u^{n+1}_{i,j}= \frac{1}{b \Delta t + 2} (\\ 2 \frac{\Delta t^2}{\Delta x^2}[q_{i+.5,j}(u^{n}_{i+1,j}-u^{n}_{i,j})-q_{i-.5,j}(u^{n}_{i,j}-u^{n}_{i-1,j})] + \\ 2 \frac{\Delta t^2}{\Delta y^2}[q_{i,j+.5}(u^{n}_{i,j+1}-u^{n}_{i,j})-q_{i,j-.5}(u^{n}_{i,j}-u^{n}_{i,j-1})]+ \\ 2\Delta t^2 f^{n}_{i,j} + b \Delta t u^{n-1}_{i,j}+4 u^{n}_{i,j}-2u^{n-1}_{i,j} )
        \end{multiline}
        \\ \\
